...
from utils import *
from models.VDSH_SP import VDSH_SP
...
#########################################################################################################
model = VDSH_SP('ng20', num_features, num_bits, y_dim, device, dropoutProb=0.1)
model.to(device)
...
with open('logs/VDSH_SP/loss.log.txt', 'w') as log_handle:
    log_handle.write('epoch,step,loss,reconstr_loss,z_kl_loss,v_kl_loss\n')
...
#########################################################################################################
model = VDSH_SP(dataset, num_features, num_bits, y_dim, device, dropoutProb=args.dropout, use_softmax=args.single_label)
model.to(device)
